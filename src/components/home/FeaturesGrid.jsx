  ...
  return (
    <section className='py-[65px] text-white px-4  '>
      ...
      <div className='max-w-[1200px] mx-auto lg:mt-[65px] mt-10 '>
        <div className='grid grid-cols-3 xxl:gap-10 xl:gap-8 lg:gap-6 gap-4 lg:mt-[60px] mt-10'>
          {
            FEATURES_GRID_DATA.map(({id,title,description,image}, index)=>{
              return(
                ...
                >
                  <div className={`rounded-3xl overflow-hidden bg-jet flex relative ${featuredGridSettings[index].cardFlexCol}
                    ${featuredGridSettings[index].cardGradientAfter} ${featuredGridSettings[index].cardGradientBefore}
                    justify-between items-center h-full lg:gap-y-[26px] md:gap-y-5 gap-y-4  `}>
                      ...
                      <div className={`gap-y-2 flex flex-col items-center text-center relative z-10 ${featuredGridSettings[index].cardContentPadding}`}>
                        <h4 className='lg:text-2xl md:text-xl text-lg text-white
